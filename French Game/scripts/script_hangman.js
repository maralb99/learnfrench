...
const getRandomWord = () => {
    // Selecting a random word from the wordList that has not been used yet
    let randomWordIndex;
    do {
        randomWordIndex = Math.floor(Math.random() * wordList.length);
    } while (usedWords.includes(randomWordIndex));

    // Marking the word as used
    usedWords.push(randomWordIndex);

    // Resetting usedWords if all words have been used
